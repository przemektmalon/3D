...
void btSimulationIslandManagerMt::buildIslands( btDispatcher* dispatcher, btCollisionWorld* collisionWorld )
{
	...
	//we are going to sort the unionfind array, and store the element id in the dimensions
...
{
    // sort islands in order of decreasing batch dimensions
    m_activeIslands.quickSort( IslandBatchSizeSortPredicate() );

    // merge small islands to satisfy minimum batch dimensions
