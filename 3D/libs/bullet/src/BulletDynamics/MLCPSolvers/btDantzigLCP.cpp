...
{  
  ...
  /* compute rows at end that are not a multiple of block dimensions */
  for (; i < n; i++) {
...
struct btLCP 
{
	...
	const int m_nskip;
	int m_nub;
	int m_nC, m_nN;				// dimensions of each index set
...
# endif
  ...
  for (int i=adj_nub; i<n; ++i) 
  {
    ...
    // set C's line has zero dimensions, so the index will always remain in set N.
    ...
    if (lo[i]==0 && w[i] >= 0) {
      ...
      for (;;) {
        ...
        // find largest step we can take (dimensions=s), either to drive x(i),w(i)
