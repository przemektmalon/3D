...
public:
	...
	void updateGLBuffers()
	{
		...
		auto totTriLists = numTriLists[Regular] + numTriLists[MultiTextured];
		MeshGPUMetaShadow* metaShadow = (MeshGPUMetaShadow*)drawIndirectBuffer[Shadow].mapRange(0, totTriLists * sizeof(MeshGPUMetaShadow), GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_WRITE_BIT);
		u32* instanceTransformsIndicesShadow = (u32*)instanceTransformsBuffer[Shadow].mapRange(0, totTriLists * sizeof(u32), GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_WRITE_BIT);
		...
		for (auto itr = instances.begin(); itr != instances.end(); ++itr)
		{
			...
			{	
				for (auto itr3 = itr2->second.begin(); itr3 != itr2->second.end(); ++itr3)
				{
					...
					auto bsptr = (*itr3)->shadowRenderMeta.batchPtr;
					auto bsindex = (*itr3)->shadowRenderMeta.batchIndex;

					auto bptr = (*itr3)->renderMeta.batchPtr; //If render meta is missing, then most likely the mesh has not been pushed to a gpu batch
					auto bindex = (*itr3)->renderMeta.batchIndex;

					metaShadow[i].cmds[0] = bptr->counts[bsindex];
					metaShadow[i].cmds[1] = 1;
					metaShadow[i].cmds[2] = bptr->firsts[bsindex];
					metaShadow[i].cmds[3] = 0;

					instanceTransformsIndicesShadow[i] = i;
