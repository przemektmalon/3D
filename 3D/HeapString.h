#pragma once
#include "Types.h"

#define min(a,b) a > b ? b : a
#define max(a,b) a > b ? a : b

class HeapString
{
public:
	HeapString() {}
	HeapString(char chr)
	{
		//expand(10);
		string = new char[10];
		replicate('\0', 10);
		string[0] = chr;


		capacity = 10;
		length = 1;
	}
	HeapString(char* chars)
	{
		u32 len = determineLength(chars);
		//expand(len);
		string = new char[len];
		capacity = len;
		length = len;

		setToChars(chars);
	}
	~HeapString() {}

	void expand(u32 pNewSize)
	{
		char* oldString = string;
		string = new char[pNewSize + 1];
		copyChars(string, oldString, length);
		capacity = pNewSize;
		HeapString nullEnd;
		nullEnd.replicate('\0', capacity - length);
		replaceAt(length, nullEnd);
		delete[] oldString;
	}

	void autoExpand()
	{
		expand(length + (length / 2));
	}

	void setToChars(char* str)
	{
		int i = 0;
		for (; i < capacity && (str[i] != '\0'); ++i)
		{
			getString()[i] = str[i];
		}
		length = i;
		for (int j = i; j < capacity; ++j)
		{
			getString()[j] = '\0';
		}
	}

	void overwrite(HeapString& source)
	{
		length = source.getLength();
		autoExpand();
		copyStr(*this, source, length);
	}

	//TODO: RETURN REFERENCE TO SELF, CHECK IF VIRTUAL SPECIALISATIONS
	HeapString& append(HeapString& str)
	{
		replaceAt(getLength(), str);
		return *this;
	}

	HeapString& append(HeapString&& str)
	{
		//copyStr(*this, str, str.getLength(), getLength());
		replaceAt(getLength(), str);
		return *this;
	}

	HeapString& append(char chr)
	{
		if (getLength() > getCapacity() - 1)
		{
			autoExpand();
		}

		getString()[getLength()] = chr;
		length++;
		return *this;
	}

	void replicate(char chr, u32 len, u32 start = 0)
	{
		if (start + len > getCapacity())
		{
			len = getCapacity() - start;
		}
		auto me = getString();
		for (int i = start; i < start + len; ++i)
		{
			me[i] = chr;
		}
	}

	void clear(HeapString& clearMask)
	{
		u32 i = 0;
		u32 maskLen = clearMask.getLength();
		while (i < getCapacity())
		{
			getString()[i] = clearMask.getString()[i % maskLen];
			++i;
		}
	}

	void operator+=(HeapString& str)
	{
		append(str);
	}

	void operator+(HeapString& str)
	{
		append(str);
	}

	bool operator==(HeapString& str)
	{
		if (str.getLength() != getLength())
			return false;

		return compareChars(getString(), str.getString(), getLength());
	}
	//void operator>
	//void operator<
	//void operator!=

	s32 contains(HeapString& str, u32 startSearch = 0)
	{
		if (str.getLength() > getLength())
			return -1;

		for (s32 i = startSearch; i < getLength() - str.getLength(); ++i)
		{
			if (compareChars(str.getString(), getString() + i, str.getLength()) == true)
			{
				return i;
			}
		}
		return s32(-1);
	}

	void moveSegmentForward(s32 takeFrom, s32 forward, u32 len, char fill = '_')
	{
		if (takeFrom + forward + len > getCapacity() - 1)
		{
			expand(takeFrom + forward + len + (getCapacity() / 2));//TODO: CORRECT EXPAND AMOUNT
			length = takeFrom + forward + len;
		}
		else
		{
			length += forward;
		}
		//TODO: FILL VOID SPACE IF ANY (LIKE IN moveSegmentBackward)

		//s32 voidLen = 

		//for(s32 i = )

		//A B C D E F G . . . .

		for (s32 i = takeFrom + forward + len - 1; i > takeFrom + forward; --i)
		{
			getString()[i] = getString()[i - forward];
		}

		getString()[length] = '\0';
	}

	void moveSegmentBackward(s32 takeFrom, s32 backward, u32 len, char fill = '_')
	{
		//if (takeFrom - backward + len > getCapacity())
		//	expand(takeFrom - backward + len + (getCapacity() / 2));//TODO: CORRECT EXPAND AMOUNT

		if (takeFrom + len > getCapacity() - 1)
			len = getCapacity() - takeFrom;
		if (takeFrom - backward + len > getCapacity() - 1)
			len = getCapacity() - (takeFrom - backward);

		for (s32 i = takeFrom - backward; i < takeFrom - backward + len; ++i)
		{
			getString()[i] = getString()[i + backward];
		}

		for (s32 i = takeFrom - backward + len; i < takeFrom + len; ++i)
		{
			getString()[i] = fill;
		}
	}

	void moveWithin(s32 takeFrom, s32 moveTo, u32 len, char fill = '_')
	{
		if (s32(moveTo) - s32(takeFrom) > 0)
			moveSegmentForward(takeFrom, moveTo - takeFrom, len, fill);
		else
			moveSegmentBackward(takeFrom, takeFrom - moveTo, len, fill);
	}

	//void map(FUNCTION (CHAR -> CHAR))
	//void mapIf(FUNCTION (CHAR -> CHAR), FUNCTION (CHAR -> BOOL));
	//void mapCase(FUNCTION (CHAR -> CHAR), String& x);
	//void mapCaseIf(FUNCTION (CHAR -> CHAR), FUNCTION (CHAR -> BOOL), String& x);
	HeapString& removeCases(HeapString&& str)
	{
		s32 i = -2;
		s32 j = 0;
		while (i != -1)
		{
			i = contains(str, j);

			if (i > -1)
			{
				moveSegmentBackward(i + str.getLength(), str.getLength(), getLength() - (i + str.getLength()), '\0');
				j = i;
				i = -2;
			}
		}
		return *this;
	}
	//void applyWhere(FUNCTION (CHAR -> CHAR), String& x)
	//StringXX findCases(String& x) { RETURN LIST OF INDICES IN STRING ENC AS BINARY }
	HeapString& insertAt(u32 i, HeapString& x)
	{
		moveSegmentForward(i, x.getLength(), getLength() - i - x.getLength());
		replaceAt(i, x);
		return *this;
	}
	HeapString& insertAt(u32 i, HeapString&& x)
	{
		moveSegmentForward(i, x.getLength(), getLength() - i - x.getLength());
		replaceAt(i, x);
		return *this;
	}

	void insertWhere(HeapString& whereStr, HeapString& insStr)
	{
		s32 i = -2;
		s32 j = 0;
		while (i != -1)
		{
			i = contains(whereStr, j);

			if (i > -1)
			{
				moveWithin(i + whereStr.getLength(), i + insStr.getLength(), getLength() - (i + whereStr.getLength()));
				replaceAt(i, insStr);
				j = i;
				i = -2;
			}
		}
	}

	void insertWhere(HeapString&& whereStr, HeapString& insStr)
	{
		s32 i = -2;
		s32 j = 0;
		while (i != -1)
		{
			i = contains(whereStr, j);

			if (i > -1)
			{
				moveWithin(i + whereStr.getLength(), i + insStr.getLength(), getLength() - (i + whereStr.getLength()));
				replaceAt(i, insStr);
				j = i;
				i = -2;
			}
		}
	}

	void insertWhere(HeapString&& whereStr, HeapString&& insStr)
	{
		s32 i = -2;
		s32 j = 0;
		while (i != -1)
		{
			i = contains(whereStr, j);

			if (i > -1)
			{
				moveWithin(i + whereStr.getLength(), i + insStr.getLength(), getLength() - (i + whereStr.getLength()));
				replaceAt(i, insStr);
				j = i;
				i = -2;
			}
		}
	}

	void insertWhere(HeapString& whereStr, HeapString&& insStr)
	{
		s32 i = -2;
		s32 j = 0;
		while (i != -1)
		{
			i = contains(whereStr, j);

			if (i > -1)
			{
				moveWithin(i + whereStr.getLength(), i + insStr.getLength(), getLength() - (i + whereStr.getLength()));
				replaceAt(i, insStr);
				j = i;
				i = -2;
			}
		}
	}

	void replaceAt(u32 i, HeapString& x)
	{
		//s32 spaceLeftAfter = (getCapacity() - i - x.getLength());
		bool overflow = (i + x.getLength()) > getCapacity() - 1;

		if (overflow)
		{
			expand(i + x.getLength() + (getCapacity() / 2));//TODO: CORRECT EXPAND AMOUNT
			length = i + x.getLength();
		}

		//s32 replaceLen = spaceLeftAfter > 0 ? x.getLength() : x.getLength() + spaceLeftAfter;
		copyChars(getString() + i, x.getString(), x.getLength());
	}

	/*StackString& replaceWhere(StackString& whereStr, StackString& repStr)
	{
	s32 i = -2;
	s32 j = 0;
	while (i != -1)
	{
	i = contains(whereStr, j);

	if (i > -1)
	{
	moveWithin(i + whereStr.getLength(), i + repStr.getLength(), getLength() - (i + whereStr.getLength()));
	replaceAt(i, repStr);
	j = i;
	i = -2;
	}
	}
	return *this;
	}*/

	u32 getCapacity() { return capacity; }
	u32 getLength() { return length; }
	char* getString() { return string; };

	static void copyStr(HeapString& dst, HeapString& src, u32 len, u32 start = 0)
	{
		if (len > src.getLength())
			len = src.getLength();

		dst.expand(src.getCapacity());

		auto dstP = dst.getString();
		auto srcP = src.getString();

		dst.length = src.length;

		for (int i = 0; i < len; ++i)
		{
			dstP[i + start] = srcP[i];
		}
	}

	//UNSAFE
	static void copyChars(char* dst, char* src, u32 len)
	{
		u32 i = 0;
		while (i < len)
		{
			dst[i] = src[i];
			++i;
		}
	}

	//UNSAFE
	static bool compareChars(char* a, char* b, u32 len)
	{
		bool ret = true;
		u32 i = 0;
		while (i < len && ret == true)
		{
			ret = (a[i] == b[i]);
			++i;
		}
		return ret;
	}

	static char* cReplicate(char chr, u32 len, u32 start = 0)
	{
		char* retChr = new char[len + start];
		for (int i = 0; i < start; ++i)
		{
			retChr[i] = ' ';
		}
		for (int i = start; i < start + len; ++i)
		{
			retChr[i] = chr;
		}
		return retChr;
	}

	//UNSAFE
	static u32 determineLength(char* str)
	{
		u32 ret = 0;
		for (;;)
		{
			if (str[ret] == '\0')
			{
				return ret;
			}
			++ret;
		}
	}

protected:

	u32 capacity;
	u32 length;

	char* string;

};

#undef min
#undef max