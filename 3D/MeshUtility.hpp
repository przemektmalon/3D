...
public:
	...
	void nullAllMeshTextures(s32 meshIndex, Texture2D* nullTex)
	{
		if (meshIndex > meshes.size() - 1)
			return;
	...
	void setMeshAlbedoTexture(s32 meshIndex, s32 triListIndex, Texture2D* texture, s32 matTexIndex)
	{
		if (meshIndex > meshes.size() - 1)
			return;
		...
		mesh->triangleLists[triListIndex].material.albedo[matTexIndex] = texture;
	}

	void setMeshSpecularTexture(s32 meshIndex, s32 triListIndex, Texture2D* texture, s32 matTexIndex)
	{
		if (meshIndex > meshes.size() - 1)
			return;
		...
		mesh->triangleLists[triListIndex].material.specular[matTexIndex] = texture;
	}

	void setMeshNormalTexture(s32 meshIndex, s32 triListIndex, Texture2D* texture, s32 matTexIndex)
	{
		if (meshIndex > meshes.size() - 1)
			return;
		...
		mesh->triangleLists[triListIndex].material.normal[matTexIndex] = texture;
	}

	void setMeshAlphaTexture(s32 meshIndex, s32 triListIndex, Texture2D* texture)
	{
		if (meshIndex > meshes.size() - 1)
			return;
		...
		mesh->triangleLists[triListIndex].material.alpha = texture;
	}
	...
	void calculateMeshRadius(s32 meshIndex)
	{
		auto mesh = validateMesh(meshIndex);
		if (!mesh)
			return;
